# ==============================================================================
# PROC Kickback Hunter AI  —  v6.1 "Agentic Single-Flow, KISS Edition"
# ------------------------------------------------------------------------------
...
# ==============================================================================
# 0. CSS (Apple Cupertino Dark Mode + Pipeline-Pills)
# ==============================================================================
def inject_css():
    st.markdown("""
    <style>
...
# ==============================================================================
THRESHOLD = 50_000_000.0   # Vertragsgrenze: ab dieser konsolidierten Summe gibt es Geld zurück
RATE = 0.02                # Rückzahlungssatz
...
PIPELINE = [
    ...
    ("🧮", "4 Summe prüfen",     "Geld zurück?",        "live"),
...
def agent_contract(token):
    ...
    log_audit(token, "Agent 3 (Vertrag lesen)", "Vertrag lesen", "Vertragsregel gefunden (simuliert)",
              f"Grenze {fmt_mio(THRESHOLD)} / {RATE*100:.0f} % zurück")
    return token
...
def agent_finance(token):
    ...
    log_audit(token, "Agent 4 (Summe prüfen)", "Summe prüfen", "Summe gebildet & mit Grenze verglichen (LIVE)",
              ...
              f"Geld zurück {fmt_curr(cashback)}")
    return token
...
def agent_booking(token):
    ...
    token["booking"] = {
        ...
        "beleg_id": beleg_id,
        "buchungstext": f"Geld zurück gutgeschrieben: {fmt_curr(fin['geld_zurueck'])}",
        "mail_draft": {
            "status": "NUR ENTWURF – wird nicht automatisch verschickt",
            "an": CFO_NAME,
            "betreff": f"Geld zurück gesichert: +{fmt_curr(fin['geld_zurueck'])} bei {token['parent']}",
    ...
    log_audit(token, "Agent 6 (Buchen)", "Buchen", "Beleg angelegt (simuliert) & E-Mail-Entwurf erstellt",
              f"Beleg #{beleg_id}; {fmt_curr(fin['geld_zurueck'])}; E-Mail = nur Entwurf")
    return token
...
<div class="frosted-glass" style="margin-bottom: 14px;">
  <h1 style="margin:0; font-size: 30px; font-weight: 800; letter-spacing:-1px;">
    PROC Kickback Hunter AI <span style="color:#007AFF; font-size:18px;">v6.1 · einfach erklärt</span>
  </h1>
  <p style="margin:10px 0 0 0; font-size:14.5px; line-height:1.6; color:#E5E5EA;">
    ...
    laut Vertrag Geld zurückbekommt – merkt es aber nicht, weil derselbe Lieferant im Computer-System
    ...
    bekommt <b>2 %</b> zurück. Im System steht Cisco aber als <b>5 getrennte Firmen</b> (verschiedene Länder,
    leicht andere Namen). Jede einzelne liegt unter 50 Mio. €, also meldet das normale System: „Kein Geld zurück.“
    ...
    <b>einen Menschen einmal final entscheiden lassen</b> → das Geld buchen. Der Nutzen: Geld, das sonst unbemerkt
    liegen bleibt, wird sichtbar und sicher zurückgeholt – und jeder Schritt ist nachvollziehbar.
  </p>
</div>
""", unsafe_allow_html=True)
...
with c1:
    ...
    target_label = st.selectbox(
        ...
        format_func=lambda x: {
            "CISCO": "Cisco – Erfolg (Geld zurück)",
...
if start:
    ...
    steps = [
        ...
        (2, agent_contract, "Ich suche die passende Regel im Vertrag …",
         lambda t: f"Regel gefunden: ab {fmt_mio(THRESHOLD)} gibt es {int(RATE*100)} % zurück."),
        (3, agent_finance, "Ich addiere alle Einkäufe und vergleiche mit der Vertragsgrenze …",
         lambda t: (f"Echte Summe {fmt_curr(t['finance']['echte_gesamtsumme'])} → "
                    f"{'Anspruch auf ' + fmt_curr(t['finance']['geld_zurueck']) if t['finance']['anspruch'] else 'kein Anspruch'}.")),
    ]
...
if token:
    ...
    with st.expander("📄 Schritt 3 · Vertrag lesen", expanded=False):
        st.markdown('<span class="tag tag-stub">SIMULIERT</span>', unsafe_allow_html=True)
        explain(
            "Ab wann es Geld zurückgibt, steht im Vertrag – meist in langen Dokumenten.",
    ...
    with st.expander("🧮 Schritt 4 · Summe prüfen", expanded=True):
        ...
        explain(
            ...
            "und reicht das für die Rückzahlung?",
        ...
        k4.metric("Geld zurück (2 %)", fmt_curr(fin["geld_zurueck"]))
    ...
    if token["status"] == "AWAITING_APPROVAL":
        ...
        verdict = (f"<b style='color:#30D158;'>Anspruch:</b> Die echte Summe "
                   ...
                   f"Vorschlag: <b>{fmt_curr(fin['geld_zurueck'])}</b> zurückbuchen."
                   ...
                   f"({fmt_mio(THRESHOLD)}). Es gibt nichts zurückzubuchen.")
        st.markdown(
    ...
    if token["status"] == "BOOKED":
        ...
        st.markdown(
            ...
            f'Mit besten Grüßen,<br><b>PROC Kickback Hunter AI</b>'
